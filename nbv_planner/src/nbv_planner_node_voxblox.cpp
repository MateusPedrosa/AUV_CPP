#include "nbv_planner/nbv_planner_node_voxblox.hpp"
...
NBVPlannerNodeVoxblox::NBVPlannerNodeVoxblox()
    : Node("nbv_planner_node_voxblox"),
      received_first_cloud_(false)
{
    ...
    this->declare_parameter("voxels_per_side", 16);
    ...
    this->declare_parameter("exploration_sensor_vfov", 20.0 * M_PI / 180.0);
    ...
    this->declare_parameter("truncation_distance", 0.4);
    this->declare_parameter("max_weight", 10000.0);
    ...
    // Configure Voxblox parameters
    VoxbloxParameters voxblox_params;
    voxblox_params.voxel_size = this->get_parameter("voxel_size").as_double();
    voxblox_params.voxels_per_side = this->get_parameter("voxels_per_side").as_int();
    voxblox_params.truncation_distance = this->get_parameter("truncation_distance").as_double();
    voxblox_params.max_ray_length = this->get_parameter("exploration_sensor_max_range").as_double();
    voxblox_params.min_ray_length = this->get_parameter("exploration_sensor_min_range").as_double();
    voxblox_params.max_weight = this->get_parameter("max_weight").as_double();
    voxblox_params.sensor_hfov = this->get_parameter("exploration_sensor_hfov").as_double();
    voxblox_params.sensor_vfov = this->get_parameter("exploration_sensor_vfov").as_double();
    
    // Initialize components
    voxblox_manager_ = std::make_shared<VoxbloxManager>(voxblox_params);
    nbv_planner_ = std::make_unique<NBVPlanner>(this->get_logger());

    // Initialize path planner action server with shared voxblox_manager
    // path_planner_ = std::make_unique<SimplePlanner>(this, voxblox_manager_);
    
    // Configure planner (placeholder)
    nbv_planner_->setParameters(voxblox_params.max_ray_length, M_PI/2, M_PI/3, 8, 3);
    ...
    tf2_filter_->registerCallback(&NBVPlannerNodeVoxblox::pointCloudCallback, this);
    ...
    planning_timer_ = this->create_wall_timer(
        ...
        std::bind(&NBVPlannerNodeVoxblox::planningTimerCallback, this));
    
    RCLCPP_INFO(this->get_logger(), "NBV Planner Node initialized");
    RCLCPP_INFO(this->get_logger(), "  Voxel size: %.3f m", voxblox_params.voxel_size);
    RCLCPP_INFO(this->get_logger(), "  Voxels per side: %d", voxblox_params.voxels_per_side);
    RCLCPP_INFO(this->get_logger(), "  Exploration sensor range: [%.2f, %.2f] m", 
                voxblox_params.min_ray_length, voxblox_params.max_ray_length);
}

void NBVPlannerNodeVoxblox::publishFrustumMarker() {
    const auto& params = voxblox_manager_->getParams();
    ...
    float range = params.max_ray_length;
...
void NBVPlannerNodeVoxblox::publishInspectionFrustumMarker(const std::string& frame_id) {
    visualization_msgs::msg::Marker marker;
    ...
    VoxbloxManager::FrustumAngles frustum_angles = voxblox_manager_->getCameraFOV(camera_intrinsics_);
...
void NBVPlannerNodeVoxblox::pointCloudCallback(const sensor_msgs::msg::PointCloud2::SharedPtr msg)
{
    ...
    try {
        ...
        // Update TSDF map using Voxblox integrator
        voxblox_manager_->insertPointCloudIntoMap(cloud, T_G_exploration_sensor);
        ...
        for (const auto& inspection_sensor_frame : inspection_sensor_frames_) {
            ...
            VoxbloxManager::FrustumAngles frustum_angles = voxblox_manager_->getCameraFOV(camera_intrinsics_);
            voxblox_manager_->markFrustumAsViewed(T_G_inspection_sensor, frustum_angles.horizontal, frustum_angles.vertical, camera_intrinsics_.max_range);
        }
        
        received_first_cloud_ = true;
        
        // Publish visualization
        publishVoxbloxMarkers();
...
void NBVPlannerNodeVoxblox::planningTimerCallback()
{
...
void NBVPlannerNodeVoxblox::publishVoxbloxMarkers()
{
    ...
    free_marker.scale.x = voxblox_manager_->getVoxelSize();
    free_marker.scale.y = voxblox_manager_->getVoxelSize();
    free_marker.scale.z = voxblox_manager_->getVoxelSize();
    ...
    occupied_marker.scale.x = voxblox_manager_->getVoxelSize();
    occupied_marker.scale.y = voxblox_manager_->getVoxelSize();
    occupied_marker.scale.z = voxblox_manager_->getVoxelSize();
    ...
    viewed_marker.scale.x = voxblox_manager_->getVoxelSize();
    viewed_marker.scale.y = voxblox_manager_->getVoxelSize();
    viewed_marker.scale.z = voxblox_manager_->getVoxelSize();
    viewed_marker.color.r = 0.0;
    viewed_marker.color.g = 0.0;
    viewed_marker.color.b = 1.0;
    viewed_marker.color.a = 1.0;
    
    voxblox::Layer<voxblox::TsdfVoxel>* tsdf_layer = voxblox_manager_->getTsdfLayer();

    // Iterate through all blocks and voxels
    ...
    for (const auto& block_index : blocks) {
        ...
        for (size_t linear_index = 0; linear_index < block->num_voxels(); ++linear_index) {
            ...
            // Only visualize voxels with some observation
            ...
            voxblox::GlobalIndex global_index = 
                voxblox::getGlobalVoxelIndexFromBlockAndVoxelIndex(
                    block_index, voxel_index, voxblox_manager_->getVoxelsPerSide());
            
            // Check if occupied (distance close to zero)
            bool is_occupied = voxblox_manager_->isOccupied(voxel_center);
            
            if (is_occupied) {
                if (voxblox_manager_->isVoxelViewed(global_index)) {
...
void NBVPlannerNodeVoxblox::publishCandidateMarkers()
{
    visualization_msgs::msg::MarkerArray marker_array;
...
int main(int argc, char** argv)
{
    ...
    auto node = std::make_shared<nbv_planner::NBVPlannerNodeVoxblox>();
    rclcpp::spin(node);
    rclcpp::shutdown();
    return 0;
}