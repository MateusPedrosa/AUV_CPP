...
{
public:
    SonarPointCloud() : Node("sonar_point_cloud_bluerov") {
        // Parameters
        this->declare_parameter("sonar_topic", "/sonar/echo/data");
...
private:
    ...
    void publishPolarImage() {
        cv::Mat grid;
        rclcpp::Time stamp;
        {
            std::lock_guard<std::mutex> lock(grid_mutex_);
            if (latest_intensity_grid_.empty()) return;
            grid  = latest_intensity_grid_.clone();
            stamp = latest_stamp_;
        }

        int rows = grid.rows;
        double range_res = (max_range_ - min_range_) / static_cast<double>(rows);
        cv::Mat polar = renderPolarImage(grid, -h_fov_ / 2.0, h_fov_ / 2.0,
                                         range_res, max_range_);
        auto msg = cv_bridge::CvImage(std_msgs::msg::Header{}, "bgr8", polar).toImageMsg();
        msg->header.stamp       = stamp;
        msg->header.frame_id    = "sonar_link";
        sonar_image_pub_->publish(*msg);
    }
    ...
    void convertImageToPointCloud(const sensor_msgs::msg::Image::ConstSharedPtr & img_msg) {
        ...
        // The Sonoptix ECHO driver embeds the configured range (metres) in pixel [0,0]
        // and the FOV changes with range: 120° for range ≤ 30 m, 90° otherwise.
        double ping_max_range = static_cast<double>(cv_ptr->image.at<uint8_t>(0, 0));
        if (ping_max_range > 0.0) {
            max_range_ = ping_max_range;
            h_fov_ = (max_range_ <= 30.0 ? 120.0 : 90.0) * M_PI / 180.0;
        }
        ...
        RCLCPP_INFO_THROTTLE(this->get_logger(), *this->get_clock(), 5000,
                        "Sonar image: %dx%d, range: %.2f-%.2fm (%.1f° FOV), range_res: %.4fm",
                        cols, rows, min_range_, max_range_, h_fov_ * 180.0 / M_PI, range_res);

        // Precompute per-column bearing trig — bearing depends only on c, not r.
        // This avoids recomputing 256 sin/cos pairs 808 times each per ping.
        const int n_arc = vertical_arc_points_;
        struct BeamTrig { double cos_b, sin_b; };
        std::vector<BeamTrig> beam_trig(cols);
        for (int c = 0; c < cols; c++) {
            double bearing = (static_cast<double>(c) / cols - 0.5) * h_fov_;
            beam_trig[c] = {std::cos(bearing), std::sin(bearing)};
        }

        // Precompute elevation trig — only changes with vertical_arc_points, not per pixel.
        struct ElevTrig { double cos_e, sin_e; };
        std::vector<ElevTrig> elev_trig(n_arc);
        for (int k = 0; k < n_arc; k++) {
            double elev = (n_arc > 1) ? (-v_fov_ / 2.0 + k * v_fov_ / (n_arc - 1)) : 0.0;
        ...
        for (int c = 0; c < cols; c++) {
            ...
            for (int r = 0; r < rows; r++) {
                ...
                for (int k = 0; k < n_arc; k++) {
                    ...
                    const double se = elev_trig[k].sin_e;
                    pcl::PointXYZI point;
                    point.x = range * ce * cb;
                    point.y = range * ce * sb;
                    point.z = range * se;
        ...
        // Flatten points_by_beam into a single cloud.
        // filterPointCloud() computes O(points × window) centroid distances but its
        // distance threshold check is currently disabled, so it is a no-op. Skip it
        // and flatten directly; re-enable filterPointCloud here when spatial filtering
        // is needed again.
        size_t total_points_before = 0;
        for (const auto& beam : points_by_beam) total_points_before += beam.size();

        pcl::PointCloud<pcl::PointXYZI> filtered_cloud;
        filtered_cloud.reserve(total_points_before);
        for (const auto& beam : points_by_beam) {
            for (const auto& pt : beam) filtered_cloud.push_back(pt);
        }
